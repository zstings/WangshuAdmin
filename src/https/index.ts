...
https.interceptors.response.use(
  (response: AppAxiosResponse<ResData>) => {
    ...
    if (code == 1001) {
      if (router.currentRoute.value.name !== 'login') {
        router.push({ name: 'login', query: { redirect: router.currentRoute.value.name as string } });
        return Promise.reject(new Error('未登录或登录状态过期'));
      }
    }
    if (code == config.successCode && config.showOKMsg) ElMessage({ message: msg, type: 'success', grouping: true, plain: true });
    if (code != config.successCode) {
      // 默认显示错误信息，除非显示设置为false
      if (config.showERRMsg) ElMessage({ message: msg, type: 'error', grouping: true, plain: true });
  ...
  error => {
    if (error.config.showERRMsg) {
      ElMessage({ message: error.message || '未知错误', type: 'error', grouping: true, plain: true });
    }
    return Promise.reject(error);
  },
);
